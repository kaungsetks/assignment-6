...
int emailValidator(char enterMail[50]);
void registration();
void lobby();
...
int emailValidator(char enterMail[50]){
    char compareMail[10] = {'@','g','m','a','i','l','.','c','o','m'};

    int numbersOfMailChar = charCounting(enterMail);

    int numberOfCompare = 9;

    for (int i = numbersOfMailChar; i > 0; i--) {
        if(enterMail[i-1] == compareMail[numberOfCompare]){
...
void registration(){
    ...
    while(checkRegisterMail == 1){
       while(validate != 0 ) {
           printf("Enter your email:");
           scanf(" %[^\n]", &info[globalIndex].email);
           emailValidator(info[globalIndex].email);
